...
def create_searchable_item(text: str) -> Optional[SearchableTradeItem]:
    if 'Rarity' not in text:
        # Probably not a POE item
        return None
...
def build_query_from_item(item: SearchableTradeItem) -> dict:
    ...
    if isinstance(item, Map):
        ...
        if item.elder:
            query['filters']['map_filters']['filters']['map_elder'] = 'true'
        if item.blighted:
...
def parse_item_info(text):
    ...
    if info['rarity'] == 'Currency':
        ...
        if info['rarity'] == 'Unique':
            print(
                "[!] " + Fore.RED + "Can't price " + Fore.WHITE + "this item because it is " + Fore.YELLOW + "unidentified" + Fore.WHITE + ". Please identify and try again.")
            return 0
        # Item Level
...
def query_trade(name: str = None, ilvl: int = None, itype=None, links=None, corrupted: bool = None, influenced=None,
                rarity=None, league: str = 'Metamorph', stats: Optional[list] = None, gem_level: int = None,
                quality: int = None, maps=None):
    """
    ...
    if (
            name == itype or rarity == 'Normal' or rarity == 'Magic' or itype == 'Metamorph') and ilvl != None:  # Unidentified item
    ...
    if stats:
        ...
        while len(j['query']['stats'][0]['filters']) > 0:
            ...
            if num_stats_ignored > (int(total_num_stats * 0.6)):
                print(
                    f"[!] Take any values after this with a grain of salt. You should probably do a" + Fore.RED + " MANUAL search")
            ...
            if 'result' in query.json():
                if (len(query.json()['result'])) == 0:
                    ...
                    print(
                        "[-] Removing the" + Fore.CYAN + f" {stat_translate(i)} " + Fore.WHITE + "mod from the list due to" + Fore.RED + " no results found.")
...
def create_pseudo_mods(j):
    ...
    if total_ele_resists > 0:
        ...
        print(
            "[o] Combining the" + Fore.CYAN + f" elemental resistance " + Fore.WHITE + "mods from the list into a pseudo-parameter")
    ...
    if total_chaos_resist > 0:
        ...
        print(
            "[o] Combining the" + Fore.CYAN + f" chaos resistance " + Fore.WHITE + "mods from the list into a pseudo-parameter")
    ...
    if total_life > 0:
        ...
        print(
            "[o] Combining the" + Fore.CYAN + f" maximum life " + Fore.WHITE + "mods from the list into a pseudo-parameter")
...
def affix_equals(text, affix):
    ...
    if text == query:
        print(
            "[+] Found mod " + Fore.GREEN + f"+{value}{text[1:]}")  # TODO: support "# to # damage to attacks" type mods and other similar
        return (True, value)
...
def watch_clipboard():
    ...
    while True:
        ...
        try:
            if text != prev:
                item = create_searchable_item(text=text)
                ...
                if item:
                    ...
                    if (info.get('rarity') == 'Unique') and (info.get('itype') != "Metamorph"):
                        ...
                        trade_info = query_trade(
                            **{k: v for k, v in info.items() if k in ('name', 'itype', 'ilvl', 'links',
                                                                      'corrupted', 'influenced', 'stats', 'rarity',
                                                                      'gem_level', 'quality', 'maps')})
                ...
                if trade_info:
                    ...
                    if len(trade_info) > 1:
                        ...
                        print(
                            "[$] Found one result with" + Fore.YELLOW + f" {price} " + Fore.WHITE + "as the price.\n\n")
